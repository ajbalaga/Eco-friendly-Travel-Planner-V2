...
## 🛠️ Tech Stack
* **Backend:** PHP 8.5+ with the [MongoDB PHP extension](https://pecl.php.net/package/mongodb) and `mongodb/mongodb` (via Composer)
* **Database:** MongoDB Atlas (document collections: `users`, `destinations`, `trips`)
* **Frontend:** Semantic HTML5, CSS3 (Custom Grid Layouts), JavaScript
* **Environment:** PHP's built-in web server (or any Apache/Nginx setup with PHP-FPM)
...
### 1. Prerequisites
* PHP 8.1+ with the `mongodb` and `openssl` extensions enabled.
...
4. Create indexes (unique email, lookup indexes) and seed the destinations collection:
   ...
   ```
   `database/ecoFriendly_travel_planner_db.sql` is kept as a historical reference to the original MySQL schema/seed data — it is no longer used at runtime.
...
3. When prompted, set the `MONGODB_URI` environment variable to your Atlas connection string (kept as a secret in Render's dashboard, never committed to git).
