...
$collection = $db->selectCollection('destinations');

$existingCount = $collection->countDocuments();
if ($existingCount > 0) {
    echo "The 'destinations' collection already has {$existingCount} document(s). Skipping seed to avoid duplicates.\n";
    echo "Delete the collection in Atlas first if you want to reseed.\n";
    exit(0);
}
...
foreach ($destinations as [$name, $location, $description, $ecoRating, $ecoNotes]) {
    $documents[] = [
...
echo 'Inserted ' . $result->getInsertedCount() . " destinations into Atlas.\n";
